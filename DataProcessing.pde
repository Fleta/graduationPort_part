...
void initializeFFTObjects(FFT[] fftBuff, float[][] dataBuffY_uV, int N, float fs_Hz) {
  ...
  for (int Ichan=0; Ichan < nchan; Ichan++) {
    ...
    fftBuff[Ichan].window(FFT.HAMMING); //bookmark8
  ...
  public void process(float[][] data_newest_uV, //holds raw EEG data that is new since the last call
    ...
    for (int Ichan=0; Ichan < nchan; Ichan++) {
      ...
      for (int I=0; I < fftBuff[Ichan].specSize(); I++) {
        prevFFTdata[I] = fftBuff[Ichan].getBand(I); //copy the old spectrum 
      }
      ...
      int tempcount = 1;
      if(isRecording && !(record_action == 0)){
        try{
          for(int I=0; I<fftBuff[Ichan].specSize();I++){
            //temp = Float.toString(fftBuff[Ichan].getBand(I)); *rounding off*"
            temp = String.format("%.5f", fftBuff[Ichan].getBand(I));
            //Integer.parseInt(temp);
            if(tempcount<121){
              recordoutput.print(", ");
              recordoutput.print(temp);
              if(tempcount%12 == 0){
                //recordoutput.print(", ");
              }
              else{
      ...
      }//print if recordbutton activated
